/* Copyright 2008 Sun Microsystems, Inc. All rights reserved. You may not modify,
...
jmaki.namespace("jmaki.widgets.jmaki.ibrowser");
...
if (!jmaki.widgets.jmaki.ibrowser.ImageScroller) {

	jmaki.widgets.jmaki.ibrowser.ImageScroller = function(baseDir,
	 divId,
	 startWidth
	 ,startHeight,
	 si) {
	   ...
	   // this is the main container div
	    ...
	    function postImageLoad(loadIntoBuffer) {
	        if (debug) {
	            jmaki.log("Try " + imageReloadTries + " " + url + " image.complete=" + imageBuffer.complete);
	        }
	        ...
	        _this.hideProgressIndicator();
	        if (loadIntoBuffer) {
	            // get the correct aspect ration here                    
	            imageLoadingPane.src = imageBuffer.src;
                    _this.adjustAspectRatio(imageBuffer,imageLoadingPane);
	        } else {
	            imagePane.src = imageBuffer.src;
                    _this.adjustAspectRatio(imageBuffer,imagePane);
	        }
                
	        // do a cross fade as long as the images aren't the same
	        if (imageLoadingPane.src != imagePane.src) {
                    // resize the image accounting for aspect ratio           
                    crossFade(0,loadIntoBuffer );
	        }
	    }

            this.adjustAspectRatio = function(buffer, pane) {
            
                    
                    var _h=0;
                    var _w = 0;
                    if (IMAGEPANE_WIDTH >= IMAGEPANE_HEIGHT) {
                        // maximize to image width
                        if (buffer.width >= buffer.height  ) {                      
                            var ratio = buffer.width /  buffer.height;
                            _h = IMAGEPANE_HEIGHT;
                            _w = IMAGEPANE_HEIGHT * ratio;                   
                        } else {                       
                            var ratio = buffer.width / buffer.height ;
                            _h = IMAGEPANE_HEIGHT;
                            _w = IMAGEPANE_HEIGHT * ratio;                  
                        ...
                        if (buffer.width < buffer.height  ) {
                            var ratio = buffer.height / buffer.width;
                            _h = IMAGEPANE_HEIGHT;
                            _w = IMAGEPANE_WIDTH * ratio;                                                 

                        } else { 
                            var ratio = buffer.height / buffer.width;
                            _w = IMAGEPANE_WIDTH;
                            _h = IMAGEPANE_WIDTH * ratio;                                                    
                        }                
                    }

                    pane.style.height = _h + "px";                     
                    pane.style.width = _w + "px"; 
                    
                    if (_h < IMAGEPANE_HEIGHT) {
                        var offsetH = (IMAGEPANE_HEIGHT - _h) / 2;
                        pane.style.top = offsetH + "px";
                    } else {
                        pane.style.top = "0px";
                    }
                    if (_w < IMAGEPANE_WIDTH) {
                        var offsetW = (IMAGEPANE_WIDTH - _w) / 2;
                        pane.style.left = offsetW + "px";                        
                    } else {
                        pane.style.left = "0px";
                    }   
	    ...
	    function loadImage(url, loadIntoBuffer) {
	        ...
	        if (loadIntoBuffer) {
	            ...
	            imageLoadingPane.onLoad = setTimeout(function(){
                        this.url=url;this.loadIntoBuffer = loadIntoBuffer;
                        postImageLoad(loadIntoBuffer,url);},0);
	        } else {
	            imageBuffer.src = url;
	    ...
	    function crossFade(count,loadIntoBuffer) {
	       ...
	       if (percentage < 100) {
	            ...
	            setTimeout(function(){
                        ...
                        crossFade(percentage,loadIntoBuffer);
                    }, 25);
	       }
	    }
	    ...
	    function minimizeInfoPane() {
	       ...
	       if (infoPaneLoop > INFOPANE_DEFAULT_HEIGHT) {
	            ...
	            infoPane.style.clip = clipMe;
	            infoPane.style.height = infoPaneLoop + "px";
	            infoPane.style.top = (tileY + (PADDING) +
                                         INFOPANE_DEFAULT_HEIGHT +
                                         IMAGEPANE_HEIGHT) - infoPaneLoop + "px";
	            if (debug) {
	                jmaki.log("minimize infoPaneLoop =" + infoPaneLoop +
	    ...
	    function drawTiles() {
	        ...
	        var overHang;
	        var tileTop = tileY + INFOPANE_DEFAULT_HEIGHT +
                              IMAGEPANE_HEIGHT + (PADDING)+ "px";
	        var temp = offset;
	        ...
	        var stopIndex = index + Math.round(VIEWPORT_WIDTH / THUMB_WIDTH) + 1;    
	        ...
	        for (var tl=startIndex; tl < stopIndex; tl++) {
	            if (debug) {
	             jmaki.log("overhang=" + overHang +  " startIndex=" + startIndex +
	            ...
	            if (overHang  > 0 && tl == startIndex) {
	                ...
	                tiles[tl].style.left = (tileX  - overHang) + 1 + "px";
	                ...
	                if (underHang > 0) {
	                    ...
	                    tiles[tl].style.clip = clipMe;
	                    tiles[tl].style.left =  tileX + (offset + (tl * THUMB_WIDTH)) + 1 + 'px';
	                    ...
	                    tiles[tl-1].style.left = tileX + (offset + ((tl -1) * THUMB_WIDTH)) + 1 + 'px';
	                } else { 
	    ...
	    function calculateSizes() {
	        ...
	        fontSize =  18;//Math.round(VIEWPORT_HEIGHT * .06);
	        ...
	        if (imageBuffer) {  
                    loadImage(imageBuffer.src, showingBuffer);
                }
         ...
         function layout() {
            ...
            infoPane.style.left = tileX + 1 + "px";
            if (maximized) {
...
jmaki.widgets.jmaki.ibrowser.Widget = function(wargs) {

        var _widget = this;
        var subscribe = ["/ibrowser", "/jmaki/ibrowser"];
	...
	function Controller() {
	  ...
	  function testInnerHTML(infoName) {
                if (navigator.userAgent.indexOf('IE') != -1) return true;
	        var testString = "   ";
	  ...
	  */
	  ...
	  this.loadJSON = function(service) {
              _widget.scroller.reset();
              // for actions
              if (service && service.message) service = service.message;
              if (service.items instanceof Array ||
	  ...
	  this.loadFlickr = function(tags, scroller, apikey) {
		  var  fl;
		  if (typeof apikey != 'undefined' && apikey != '') fl = new FlickrLoader(apikey);
	...
	if (typeof wargs.args != 'undefined') {
        ...
        if (wargs.args) {
            if (wargs.args.width) startWidth = wargs.args.width;
            if (wargs.args.height)startHeight = wargs.args.height;  
        }
        
        
	_widget.scroller = new jmaki.widgets.jmaki.ibrowser.ImageScroller(wargs.widgetDir,
                       wargs.uuid, startWidth, startHeight,scaleIcons);
	_widget.scroller.callbackHandler = controller.handleEvent;
	
	_widget.resize = _widget.scroller.resize;
	...
	var apikey;
	var tags='thekt';
	if (wargs.args &&  wargs.args.sourceType != 'undefined') {
